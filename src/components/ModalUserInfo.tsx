...
const ModalUserInfo: React.FC<ModalLoginRegisterProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed left-0 h-100vh bg-black/80 top-0 bottom-0 right-0 z-50">
      ...
      >
        <XMarkIcon className="h-10 w-10 z-50 text-white border p-1 rounded-full" />
      </div>
      <div className="bg-stone-100 z-50 mt-16 max-h-[70%] overflow-y-scroll">
        <h4 className="uppercase text-mainColor font-bold text-center text-xl py-3">
          Thông tin tài khoản
        </h4>
        <div className="mx-4 bg-[#0000001a] px-6 py-5 rounded-xl font-semibold flex flex-col gap-4">
          <div>
            Giới tính:{" "}
          ...
          <div>
            Email: <span className="text-gray-600 ml-1">{userInfo?.email}</span>
          </div>
        </div>
        {openEditInfo ? (
          <></>
        ) : (
          <div className="uppercase text-[#f18720] font-bold text-lg px-4 py-2 bg-[#0000001a] my-3 w-[60%] mx-auto text-center rounded-xl">
        ...
        <div>
          <div className="flex justify-between gap-0.5 px-3 mx-auto">
            {listAction.map((item, index) => (
              <div
                className="px-3 py-2 bg-[#969696] text-sm font-bold uppercase text-stone-100 rounded-md"
                key={index}
              >
                {item.title}
              </div>
        ...
        <div id="infoUpdate">
          {openEditInfo && (
            <React.Fragment>
              <p className="text-gray-500 italic text-lg font-semibold text-center">
